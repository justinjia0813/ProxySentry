...
- 网络状态变化时短暂展开并立即复检；稳定状态下每 60 秒后台检查只更新状态，不主动展开。
...
- 浅色、深色、长中文、异常溢出和网络变化展开均保持可读；悬停和收起不造成输入焦点丢失或卡死。
- VoiceOver（macOS 屏幕阅读器）可以读出状态、矩阵和操作，键盘可以到达全部操作；不需要辅助功能权限。外部点击只通过公开的只读鼠标事件监视器观察，不拦截或修改其他应用输入。
- 空闲时没有忙循环或重叠诊断；60 秒定时检查不触发展开、不重复通知；网络变化触发的展开约 5 秒后收起，鼠标仍在面板内时不强制收起。
...
| 状态 | 触发 | 用户可见行为 | 退出 |
| --- | --- | --- | --- |
| 收纳 | 无 hover、无近期网络变化 | 顶部胶囊保持最小宽度，仅显示状态符号和短标签 | 鼠标悬停或网络变化 |
| hover 展开 | 鼠标进入浮窗的本地 tracking area | 展开终端风格面板，显示四行矩阵、主结论和最多两条异常证据 | 鼠标离开、外部点击或 Escape |
| 网络变化展开 | `NWPathMonitor` 或系统代理观察到变化 | 展开约 5 秒并立即复检，显示检测中状态；不改变 60 秒周期 | 计时结束、用户离开或外部点击；鼠标仍在面板内时不强制收起 |
| 60 秒检查 | 稳定状态定时器 | 后台更新模型，不自动展开、不抢焦点 | 下一次状态变化或用户 hover |
| 外部点击/Escape | 浮窗外点击或 Escape | 立即收起，不取消正在进行的诊断 | 下一次 hover 或网络变化 |
...
- 自动测试确认测试宿主不创建全局浮窗；hover、离开、外部点击、Escape、网络变化展开和 60 秒定时不展开由下述实机验收覆盖。
...
实机验收：刘海 MacBook、无刘海外接屏、多屏、全屏 Space、浅色/深色、VoiceOver、键盘操作、鼠标离开、外部点击、Escape、网络变化短暂展开和至少一个 60 秒稳定窗口。
