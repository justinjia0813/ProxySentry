...
- 关闭状态显示一个紧凑的刘海胶囊；鼠标悬停时展开，展示 `base / traffic / proxy / clash` 四行终端矩阵。
- 网络变化只触发立即复检，不自动展开；黄、红或灰状态连续至少 3 分钟时短暂展开一次，恢复正常后才允许下一次提醒。
...
- 交互验收：hover 展开、短时网络变化不展开、持续故障仅展开一次、鼠标离开/外部点击/Escape 收起。
